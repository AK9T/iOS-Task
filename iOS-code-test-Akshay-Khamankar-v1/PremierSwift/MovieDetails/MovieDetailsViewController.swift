...
final class MovieDetailsViewController: UIViewController {
    ...
    private func updateFromViewModel() {
        ...
        case .loaded(let details, let similarMovies):

            if let movieDetails = details as? MovieDetails, let similarDetails = similarMovies as? SimilarMovieDetails {
                self.showMovieDetails(movieDetails, similarDetails.results ?? [])
            }
        case .error:
            self.showError()
        }
    ...
    private func showMovieDetails(_ movieDetails: MovieDetails, _ similarMovies: [SimilarMovieDetailsResult]) {
        let displayViewController = MovieDetailsDisplayViewController(
